...
- **A connected USB cable blocks deep suspend entirely.** `echo mem` reports success but the SoC never stays down (returns in ~1-2s), so the daemon would hot-spin a full fetch+redraw loop and drain ~10%/hr. `suspend_for()` now measures the suspend and falls back to a userspace sleep when it didn't stick ([D22]). Diagnostic: `cat /sys/class/power_supply/max77696-charger/online` (USB present) and `max77696-eh/online` (Mains charger present). A real charger shows `eh`/Mains `online=1` + `isCharging=1`; a bare data port shows `USB_CDP` + `isCharging=0` — plugged in, blocks sleep, *doesn't* charge. This is the **M4.6** power-delivery constraint.
