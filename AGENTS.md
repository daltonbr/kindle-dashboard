...
| **M7** — Rolling month tile | ✅ code-complete (deploy + verify pending). `CalendarMonth` card fills the bottom-right cell: a Monday-first rolling 4-week grid (today highlighted, event-days dotted, spanning month label) plus a "later" footer showing the next events beyond the agenda's window — both tiles read the same `CalendarModel` (shared `defaultAgendaEvents`) so they complement, not duplicate. Pure render layer, no new provider/secret. See `docs/roadmap.md` M7 |
