...
- **Main loop:** `suspend_for(interval)` (arm `rtc1/wakealarm` → `echo mem > /sys/power/state`) → on resume, LIPC wireless nudge (`wirelessEnable 0; sleep 2; wirelessEnable 1`) → poll-curl up to 20s → PNG magic check → `eips -g` → screensaver copy → battery sample to `batt.csv` → log trim → fast-return guard.
- **`suspend_for(secs)` — the only safe way to idle:** all non-trivial waits go through it. A plain userspace `sleep` is **not** safe for the daemon: busybox `sleep` is `nanosleep()` on `CLOCK_MONOTONIC`, which does not advance while the device is suspended, and `powerd` (which we don't stop) idle-suspends the panel after a few minutes. A userspace sleep longer than powerd's idle timeout therefore freezes mid-count and never returns — that stalled the whole loop for ~8h on 2026-06-14 (see [D21](decisions.md)). `suspend_for` keeps the suspend ours and the wake a hardware RTC alarm.
- **Fast-return guard:** if a cycle finishes in less than `INTERVAL/2` (typical cause: a USB plug or user tap waking us early), do a short 5s userspace cooldown (rides out a transient asserted wake source — too brief for powerd to act) then `suspend_for` the remainder. Defends against hot-spinning on stray wakeup sources **without** ever sitting in a long userspace sleep.
...
- **Physically wake it** (USB plug/unplug or a tap/power-button press is a wakeup source). The daemon resumes, brings Wi-Fi up, runs its cycle, and re-suspends. Note since [D21](decisions.md): an early wake no longer holds the device awake — the fast-return guard re-suspends after a 5s cooldown — so a single tap only buys the ~10–20s awake window, not a long one. Use it to land your `touch state/maintenance` (see below), not to work directly.
- **Maintenance mode** is the deliberate "stay awake so I can work" switch (`touch state/maintenance`), but it requires ssh access to set — so reach for the physical wake first, then immediately `touch state/maintenance` to hold the device open. If you keep missing the short awake window, script a tight reconnect loop that retries every few seconds and runs the `touch` the instant it lands (that's how the device is recovered in practice).
